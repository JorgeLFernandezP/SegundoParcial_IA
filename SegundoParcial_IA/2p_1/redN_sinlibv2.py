...
if __name__ == "__main__":
    ...
    wh, wo, losses = train(X, y, hidden_size, epochs, learning_rate)
    predictions = predict(X, wh, wo)
    accuracy = np.mean(np.argmax(y, axis=1) == predictions)
    
    print(f"Accuracy: {accuracy * 100}%")
    
    # Encontrar la época con la pérdida mínima
    min_loss_epoch = np.argmin(losses)
    min_loss_value = losses[min_loss_epoch]
    print(f"Minimum loss of {min_loss_value} was achieved at epoch {min_loss_epoch}")
