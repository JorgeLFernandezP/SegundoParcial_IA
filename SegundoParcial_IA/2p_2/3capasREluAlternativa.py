...
def initialize_weights_xavier(input_size, hidden_size1, hidden_size2, output_size):
    wh1 = np.random.randn(input_size, hidden_size1) * np.sqrt(2.0 / (input_size + hidden_size1))
    wh2 = np.random.randn(hidden_size1, hidden_size2) * np.sqrt(2.0 / (hidden_size1 + hidden_size2))
    wo = np.random.randn(hidden_size2, output_size) * np.sqrt(2.0 / (hidden_size2 + output_size))
    return wh1, wh2, wo
...
def backward_propagation_l2(X, y, hidden_output1, hidden_output2, output_output, wh1, wh2, wo, learning_rate, reg_lambda):
    ...
    wo -= reg_lambda * wo  # Regularización L2
    
    wh2 += hidden_output1.T.dot(hidden_delta2) * learning_rate
    wh2 -= reg_lambda * wh2  # Regularización L2
    
    wh1 += X.T.dot(hidden_delta1) * learning_rate
    wh1 -= reg_lambda * wh1  # Regularización L2
    
    return wh1, wh2, wo

# Entrenamiento de la red con cambios
def train(X, y, hidden_size1, hidden_size2, epochs, learning_rate, reg_lambda):
    input_size = X.shape[1]
    output_size = y.shape[1]
    wh1, wh2, wo = initialize_weights_xavier(input_size, hidden_size1, hidden_size2, output_size)
    
    for epoch in range(epochs):
        hidden_output1, hidden_output2, output_output = forward_propagation(X, wh1, wh2, wo)
        wh1, wh2, wo = backward_propagation_l2(X, y, hidden_output1, hidden_output2, output_output, wh1, wh2, wo, learning_rate, reg_lambda)
...
if __name__ == "__main__":
    ...
    wh1, wh2, wo = train(X, y, hidden_size1, hidden_size2, epochs, learning_rate, reg_lambda)
